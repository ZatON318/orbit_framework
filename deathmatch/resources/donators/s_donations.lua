		...
		if (gameAccountID > 0) then
			...
			if (tonumber(perkID) == 14) then -- Add an int slot
				if (characterID and tonumber(characterID) > 0) then
					...
					dbExec(exports.mysql:getConn("mta"), "UPDATE `accounts` SET `credits`=credits-? WHERE `id`=?", points, gameAccountID)
			...
			elseif (tonumber(perkID) == 16) then -- Username change permit
				if (characterID and tonumber(characterID) > 0) then
					...
					local mysqlQ = dbQuery(exports.mysql:getConn("mta"), "SELECT `username` FROM `accounts` WHERE `username` = ?", username)
					...
					dbExec(exports.mysql:getConn("mta"), "UPDATE `accounts` SET `username`=?, `credits`=credits-? WHERE `id`=?", username, points, gameAccountID)
					setElementData(targetPlayer, "credits", curGC-points)
					loadAllPerks(targetPlayer)
...
function takeGC(thePlayer, amount)
	if not amount or not tonumber(amount) or tonumber(amount) <= 0 then
		...
		local currentGC = dbQuery(exports.mysql:getConn("mta"), "SELECT `credits` FROM `accounts` WHERE `id`=?  LIMIT 1", id)
...
function giveAccountGC(account, amount, historyNote)
	...
	elseif not account or not tonumber(account) or tonumber(account) <= 0 then
		...
		dbExec( exports.mysql:getConn('mta'), "UPDATE accounts SET credits=credits+? WHERE id=? ", amount, account )
		if historyNote then
