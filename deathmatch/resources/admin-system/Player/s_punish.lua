...
function offlinePunishPlayer(thePlayer, commandName, target, points, repetitive, ...)
	if exports.integration:isPlayerTrialAdmin( thePlayer ) then
		...
		local qh = dbQuery(exports.mysql:getConn('mta'), "SELECT id, username, ip, punishdate, punishpoints FROM accounts WHERE `username`=? LIMIT 1", target)
...
function sqlAddPoints(player, accountID, newpoints)
	if tonumber(accountID) and tonumber(newpoints) and player then
		if dbExec( exports.mysql:getConn('mta'), "UPDATE `accounts` SET `punishpoints`=? WHERE `id`=?", newpoints, accountID ) then
			return true
		end
	else
...
function sqlSetDate(accountID)
	if accountID then
		if dbExec( exports.mysql:getConn('mta'), "UPDATE `accounts` SET `punishdate`=NOW() WHERE `id`=?", accountID ) then
			return true
		end
	else
		return false
...
function checkExpiration(thePlayer, accountID)
	...
	local qh = dbQuery(exports.mysql:getConn('mta'), "SELECT id, punishpoints, punishdate, TIMESTAMPDIFF(DAY,punishdate,NOW()) AS date FROM accounts WHERE `id`=? LIMIT 1", accountID)
	...
	if sqlHandler and #sqlHandler > 0 and sqlHandler[1]['id'] then
		...
		if toBeRemoved >= 1 then
			...
			dbExec( exports.mysql:getConn('mta'), "UPDATE `accounts` SET `punishpoints`=?, `punishdate`=DATE_ADD(punishdate, INTERVAL ? DAY) WHERE `id`=?", newPointTotal, tonumber(45*toBeRemoved), accountID )
			if thePlayer then
