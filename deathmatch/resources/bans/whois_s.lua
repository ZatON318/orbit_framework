...
local function showIPAlts(thePlayer, ip)
	dbQuery(
		...
		end, {thePlayer, ip}, mysql:getConn("mta"), "SELECT `id`, `username` FROM `accounts` WHERE `ip`=? LIMIT 1", ip)
end
		...
		if not (...) then
			...
			if not targetPlayer or getElementData( targetPlayer, "loggedin" ) ~= 1 then
				...
				if char_result and char_result["account"] then
					...
					end, {thePlayer}, mysql:getConn("mta"), "SELECT ip FROM accounts WHERE id=? LIMIT 1", char_result["account"] )
				...
				end, {thePlayer}, mysql:getConn("mta"), "SELECT ip FROM accounts WHERE username = ?", targetPlayerName )
				...
				end, {thePlayer}, mysql:getConn("mta"), "SELECT ip FROM accounts WHERE ip = ?", targetPlayerName )
	...
	if exports.integration:isPlayerTrialAdmin( thePlayer ) then
		if not (...) then
			...
			if not targetPlayer then --or getElementData( targetPlayer, "loggedin" ) ~= 1 then
				...
				end, {thePlayer}, mysql:getConn("mta"), "SELECT `id` FROM accounts WHERE ip=?", targetPlayerName)
