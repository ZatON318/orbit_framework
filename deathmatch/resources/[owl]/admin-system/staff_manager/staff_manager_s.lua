...
function getStaffInfo(username, error)
	...
	end, {username, error, source}, exports.mysql:getConn("mta"), "SELECT id, username, admin, supporter, vct, scripter, mapper, fmt FROM accounts WHERE username=?", username)
end
...
function getTeamsData()
	...
	, {staffTitles, users}, exports.mysql:getConn("mta"), "SELECT id, username, admin, supporter, vct, scripter, mapper, fmt FROM accounts  WHERE admin > 0 OR supporter > 0 OR vct > 0 OR scripter>0 OR mapper>0 OR fmt>0 GROUP BY id ORDER BY admin DESC, supporter DESC, vct DESC, scripter DESC, mapper DESC")
end
...
function editStaff(userid, ranks, details)
		...
		if result then
			...
			if ranks[1] and ranks[1] ~= tonumber(user.admin) then
				...
				if ranks[1] == 0 then -- Remove all tickets if they get removed from admin
					dbExec(exports.mysql:getConn("mta"), "UPDATE `tc_tickets` SET `assign_to`=NULL WHERE `assign_to`=?", userid)
				end
			end
			...
			if tail ~= '' then
				...
				if not dbExec(mysql:getConn("mta"), dbPrepareString(mysql:getConn("mta"), "UPDATE accounts SET " .. tail .. " WHERE id=" .. userid)) then
	...
	end, {userid, staffTitles, target, userid, ranks, details, thePlayer}, exports.mysql:getConn("mta"), "SELECT id, username, admin, supporter, vct, scripter, mapper, fmt FROM accounts WHERE id=?", userid)
