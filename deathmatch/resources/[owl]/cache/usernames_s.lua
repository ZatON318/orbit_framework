...
function getUsername( clue )
	...
	if not searched[clue] or getTickCount() - searched[clue] > refreshCacheRate*1000*60 then
		...
		local qh = dbQuery(exports.mysql:getConn("mta"), "SELECT `id`,`username` FROM accounts WHERE `username`=? LIMIT 1", clue)
...
function getIdFromUsername(username)
	...
	local qh = dbQuery(exports.mysql:getConn("mta"), "SELECT `id` FROM accounts WHERE `username`=? LIMIT 1", username)
	local query = dbPoll(qh, 10000)
	if query and #query > 0 then
...
function getUsernameFromId(id)
	...
	local qh = dbQuery(exports.mysql:getConn("mta"), "SELECT `username` FROM accounts WHERE `id`=? LIMIT 1", id)
	local query = dbPoll(qh, 10000)
	if query and #query > 0 then
...
function startUp()
	dbQuery( 
		...
		end, exports.mysql:getConn("mta"), "SELECT id, username FROM accounts")
end
