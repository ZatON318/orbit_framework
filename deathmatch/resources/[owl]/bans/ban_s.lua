			...
			elseif (hours>168) then
				...
				if (targetPlayerPower <= thePlayerPower) then -- Check the admin isn't banning someone higher rank them him
					...
					if banId and tonumber(banId) then
						banQ = dbQuery(mysql:getConn("mta"), "SELECT * FROM bans WHERE id=? LIMIT 1", banId)
...
function offlineBanAPlayer(thePlayer, commandName, targetUsername, hours, ...)
	if exports.integration:isPlayerTrialAdmin(thePlayer) then
		if not (targetUsername) or not (hours) or not tonumber(hours) or (tonumber(hours)<0) or not (...) then
			...
			local qh = dbQuery(mysql:getConn("mta"), "SELECT * FROM `accounts` WHERE `username`=? LIMIT 1", targetUsername)
			...
			if result and #result > 0 then
				...
				if user and user['id'] and tonumber(user['id']) then
					local banQ = dbQuery(mysql:getConn("mta"), "SELECT * FROM bans WHERE account=? AND (until IS NULL OR until > NOW() ) LIMIT 1", result[1].id)
	...
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not serial or not string.len(serial) or not string.len(serial) == 32 or not (...) then
			...
			if id and tonumber(id) then
				banQ = dbQuery(mysql:getConn("mta"), "SELECT * FROM bans WHERE id=? LIMIT 1", id)
				ban = dbPoll(banQ, 10000)
...
function banPlayerAccount(thePlayer, commandName, account, ...)
	...
	local banQ = dbQuery(mysql:getConn("mta"), "SELECT * FROM bans WHERE account=? ORDER BY id DESC LIMIT 1", accountid)
	local ban = dbPoll(banQ, 10000)
	if ban and #ban == 1 and tonumber(ban[1]['id']) then
		lastBan = ban[1]
		if lastBanTimer and isTimer(lastBanTimer) then
		...
		if not id or not tonumber(id) then
			...
			if tonumber(getElementData(thePlayer, "cmd:unban")) ~= tonumber(id) then
				...
				if ban and #ban == 1 and ban[1]['id'] and tonumber(ban[1]['id']) then
					...
					if dbExec(mysql:getConn("mta"), "DELETE FROM bans WHERE id=?", id) then
...
function checkForSerialOrIpBan(playerNick, playerIP, playerUsername, playerSerial, playerVersionNumber, playerVersionString)
	--serial + IP ban.
	local resultQ = dbQuery(mysql:getConn("mta"), "SELECT * FROM bans WHERE (mta_serial=? OR ip=?) AND (until IS NULL OR until > NOW() ) LIMIT 1", playerSerial, playerIP)
	...
	--IP range ban
	resultQ = dbQuery(mysql:getConn("mta"), "SELECT * FROM bans WHERE ip LIKE '%*%' ")
	local result = dbPoll(resultQ, 10000)
	if result and #result > 0 then
...
function checkAccountBan(userid)
	local resultQ = dbQuery(mysql:getConn("mta"), "SELECT * FROM bans WHERE account=? AND (until IS NULL OR until > NOW() ) LIMIT 1", userid)
	local result = dbPoll(resultQ, 10000)
	if result and #result > 0 then
		lastBan = result
		if lastBanTimer and isTimer(lastBanTimer) then
...
function showBanDetails(thePlayer, commandName, clue)
	if exports.integration:isPlayerTrialAdmin(thePlayer) then
		if clue then
			...
			end, {thePlayer, clue, username}, mysql:getConn("mta"), "SELECT * FROM bans WHERE id=? OR mta_serial=? OR ip=? OR account=? ORDER BY date DESC", clue, clue, clue, username)
		elseif lastBan then
...
function addToBan(account, serial, ip, admin, reason, hours)
	...
	return dbExec(mysql:getConn("mta"), "INSERT INTO bans SET date=NOW() "..tail)
end

function cleanUp()
	dbExec(mysql:getConn("mta"), "DELETE FROM bans WHERE until IS NOT NULL AND until < NOW() ")
end
